...
| 04-03-03 | 03 | 2 | WHEEL-01 | T-04-05/06/07 | id-param law; unknown id → redirect; explicit-save-only; web zero-canvas | component (route) | `npx vitest run src/__tests__/explore-route.test.tsx src/__tests__/mini-wheel-card.test.tsx && npx tsc --noEmit` | ❌ W0 → created in-task | ⬜ pending |
...
| 04-07-02 | 07 | 6 | A11Y-02, A11Y-03 | — | Web zero-canvas + capability card; never-color-alone; font scaling preserved | component (Platform mock) | `npx vitest run src/__tests__/explore-web.test.tsx src/__tests__/wheel-a11y-parity.test.tsx` | ❌ W0 → created in-task | ⬜ pending |
...
| Web capability card visual | A11Y-03 | Browser check beyond the Platform-mock test | 04-07-PLAN Task 3 step 7 |
